define(['dart_sdk'], function(dart_sdk) {
  ...
  const menu = Object.create(dart.library);
  const slider = Object.create(dart.library);
  const dialog$ = Object.create(dart.library);
  const ui = Object.create(dart.library);
  const dial = Object.create(dart.library);
  ...
  dart.setLibraryUri(menu.DropdownMenu, "package:ui/src/menu.dart");
  const _dragging = dart.privateName(slider, "_dragging");
  const _value = dart.privateName(slider, "_value");
  const _disabled = dart.privateName(slider, "_disabled");
  const _touchY = dart.privateName(slider, "_touchY");
  const _line = dart.privateName(slider, "_line");
  const _knob = dart.privateName(slider, "_knob");
  const _knobGroup = dart.privateName(slider, "_knobGroup");
  const _knobImage = dart.privateName(slider, "_knobImage");
  const _updateDrawing = dart.privateName(slider, "_updateDrawing");
  const parent = dart.privateName(slider, "MixSlider.parent");
  const width$ = dart.privateName(slider, "MixSlider.width");
  const height$ = dart.privateName(slider, "MixSlider.height");
  const minValue = dart.privateName(slider, "MixSlider.minValue");
  const maxValue = dart.privateName(slider, "MixSlider.maxValue");
  const step = dart.privateName(slider, "MixSlider.step");
  const unit = dart.privateName(slider, "MixSlider.unit");
  const onChanged = dart.privateName(slider, "MixSlider.onChanged");
  const onAdjusted = dart.privateName(slider, "MixSlider.onAdjusted");
  const ticks = dart.privateName(slider, "MixSlider.ticks");
  slider.MixSlider = class MixSlider extends core.Object {
    ...
    valueToY(v) {
      return dart.notNull(this.y0) + (1.0 - dart.notNull(core.num._check(v))) * dart.notNull(this.innerHeight);
    }
    yToValue(y) {
      return 1.0 - dart.notNull(core.num._check(dart.dsend(dart.dsend(dart.dsend(y, '-', [this.y0]), '/', [this.innerHeight]), 'clamp', [0.0, 1.0])));
    }
    get disabled() {
    ...
    buildUI(container) {
      ...
      for (let tick of this.ticks) {
        let t = (t0 = svg.TextElement.new(), t0[$attributes] = new (IdentityMapOfString$String()).from(["x", dart.str(dart.notNull(this.width) / 2 - 8), "y", dart.str(this.valueToY(tick[$_get]("value"))), "class", "slider-tick-label"]), t0);
  ...
  (slider.MixSlider.new = function(width, height) {
    ...
    this[width$] = width;
    this[height$] = height;
    this.parent = (t0 = svg.SvgSvgElement.new(), t0[$attributes] = new (IdentityMapOfString$String()).from(["viewBox", "0 0 " + dart.str(this.width) + " " + dart.str(this.height), "preserveAspectRatio", "none"]), t0);
    this[_line] = (t0$ = svg.LineElement.new(), t0$[$attributes] = new (IdentityMapOfString$String()).from(["class", "slider-track", "x1", dart.str(dart.notNull(this.width) / 2), "y1", dart.str(this.y0), "x2", dart.str(dart.notNull(this.width) / 2), "y2", dart.str(this.y1)]), t0$);
    this[_knobGroup] = svg.GElement.new();
    this[_knob] = (t0$0 = svg.CircleElement.new(), t0$0[$attributes] = new (IdentityMapOfString$String()).from(["class", "slider-knob", "cx", "0", "cy", "0", "r", dart.str(this.knobRadius)]), t0$0);
    this[_knobImage] = (t0$1 = svg.ImageElement.new(), t0$1[$attributes] = new (IdentityMapOfString$String()).from(["x", "-" + dart.str(dart.notNull(this.knobRadius) * 0.7), "y", "-" + dart.str(dart.notNull(this.knobRadius) * 0.7), "width", dart.str(dart.notNull(this.knobRadius) * 1.4), "height", dart.str(dart.notNull(this.knobRadius) * 1.4), "pointer-events", "none", "href", "/images/instruments/synth.svg"]), t0$1);
  ...
  dart.setLibraryUri(slider.MixSlider, "package:ui/src/slider.dart");
  ...
  dialog$.Dialog = class Dialog extends core.Object {
    ...
    static openModal(templateSelector, fieldValues) {
      if (fieldValues === void 0) fieldValues = null;
      let template = html.TemplateElement._check(html.querySelector(templateSelector));
      if (template != null) {
        let frag = html.DocumentFragment._check(template.content[$clone](true));
        let dialog = html.DivElement._check(frag.querySelector(".dialog"));
        if (dialog != null && fieldValues != null) {
          for (let selector of fieldValues[$keys]) {
            let el = html.HtmlElement._check(dialog.querySelector(selector));
  ...
  dart.setLibraryUri(dialog$.Dialog, "package:ui/src/dialog.dart");
  const _dragging$ = dart.privateName(dial, "_dragging");
  const _value$ = dart.privateName(dial, "_value");
  const _disabled$ = dart.privateName(dial, "_disabled");
  const _touchY$ = dart.privateName(dial, "_touchY");
  const _arc = dart.privateName(dial, "_arc");
  const _line$ = dart.privateName(dial, "_line");
  const _track = dart.privateName(dial, "_track");
  const _describeArc = dart.privateName(dial, "_describeArc");
  const _updateDrawing$ = dart.privateName(dial, "_updateDrawing");
  const name$ = dart.privateName(dial, "EffectsDial.name");
  const width = dart.privateName(dial, "EffectsDial.width");
  const height = dart.privateName(dial, "EffectsDial.height");
  const minValue$ = dart.privateName(dial, "EffectsDial.minValue");
  const maxValue$ = dart.privateName(dial, "EffectsDial.maxValue");
  const step$ = dart.privateName(dial, "EffectsDial.step");
  const values = dart.privateName(dial, "EffectsDial.values");
  const unit$ = dart.privateName(dial, "EffectsDial.unit");
  const onChanged$ = dart.privateName(dial, "EffectsDial.onChanged");
  const onAdjusted$ = dart.privateName(dial, "EffectsDial.onAdjusted");
  const knob = dart.privateName(dial, "EffectsDial.knob");
  dial.EffectsDial = class EffectsDial extends core.Object {
    ...
    buildUI(container) {
      ...
      let parent = (t0 = svg.SvgSvgElement.new(), t0[$attributes] = new (IdentityMapOfString$String()).from(["width", "100%", "viewBox", "0 0 " + dart.str(this.width) + " " + dart.str(this.height), "preserveAspectRatio", "none"]), t0);
      ...
      this.knob = (t0$1 = svg.CircleElement.new(), t0$1[$attributes] = new (IdentityMapOfString$String()).from(["cx", "0", "cy", "0", "r", dart.str(this.knobRadius), "fill", "none", "stroke", "#fff8", "stroke-opacity", "1", "stroke-width", "4"]), t0$1);
      knobGroup[$nodes][$add](this.knob);
      knobGroup[$nodes][$add](this[_track]);
      knobGroup[$nodes][$add](this[_arc]);
      knobGroup[$append](this[_line$]);
      let overlay = (t0$2 = svg.CircleElement.new(), t0$2[$attributes] = new (IdentityMapOfString$String()).from(["cx", "0", "cy", "0", "r", dart.str(this.trackRadius), "fill", "white", "fill-opacity", "0.0"]), t0$2);
    ...
    [_describeArc](x, y, radius, startAngle, endAngle) {
      ...
      return JSArrayOfObject().of(["M", ex, ey, "A", radius, radius, 0, large, 1, sx, sy])[$join](" ");
    }
    [_updateDrawing$]() {
      if (this[_line$] != null) {
  ...
  (dial.EffectsDial.new = function(name) {
    ...
    this[_track] = null;
    this[knob] = null;
    this[_track] = (t0 = svg.PathElement.new(), t0[$attributes] = new (IdentityMapOfString$String()).from(["class", "dial-track", "d", this[_describeArc](0, 0, this.trackRadius, 0, this.sweep)]), t0);
    this[_arc] = (t0$ = svg.PathElement.new(), t0$[$attributes] = new (IdentityMapOfString$String()).from(["class", "dial-track-fill", "d", this[_describeArc](0, 0, this.trackRadius, 0, dart.notNull(this[_value$]) * dart.notNull(this.sweep))]), t0$);
    this[_line$] = (t0$0 = svg.LineElement.new(), t0$0[$attributes] = new (IdentityMapOfString$String()).from(["class", "dial-pointer", "x1", "0", "y1", "0", "x2", dart.str(this.polarX(this[_value$], this.knobRadius)), "y2", dart.str(this.polarY(this[_value$], this.knobRadius))]), t0$0);
    this.name = name;
  ...
  dart.setLibraryUri(dial.EffectsDial, "package:ui/src/dial.dart");
  ...
  const _button = dart.privateName(dial, "_button");
  const _color = dart.privateName(dial, "_color");
  const cx$ = dart.privateName(dial, "ToggleButton.cx");
  const cy$ = dart.privateName(dial, "ToggleButton.cy");
  const radius$ = dart.privateName(dial, "ToggleButton.radius");
  const onChanged$0 = dart.privateName(dial, "ToggleButton.onChanged");
  dial.ToggleButton = class ToggleButton extends core.Object {
    ...
    buildUI(container) {
      let t0;
      let outline = (t0 = svg.CircleElement.new(), t0[$attributes] = new (IdentityMapOfString$String()).from(["cx", dart.str(this.cx), "cy", dart.str(this.cy), "r", dart.str(this.radius), "stroke", "white", "stroke-width", "2", "stroke-opacity", "0.5"]), t0);
  ...
  (dial.ToggleButton.new = function(cx, cy, radius) {
    ...
    this[cy$] = cy;
    this[radius$] = radius;
    this[_button] = (t0 = svg.CircleElement.new(), t0[$attributes] = new (IdentityMapOfString$String()).from(["cx", dart.str(this.cx), "cy", dart.str(this.cy), "r", dart.str(dart.notNull(this.radius) - 3), "fill", "cyan", "class", "mute-button"]), t0);
  ...
  dart.setLibraryUri(dial.ToggleButton, "package:ui/src/dial.dart");
